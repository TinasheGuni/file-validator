...
QString ConsoleCount::processFile(QString f) {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    result = QString("%1 did not open successfully").arg(f);
  } else {
    QTextStream in(&file);
    ...
    if (aFlag)
      result.append(process('a', str));
    if (bFlag)
      result.append(process('b', str));
    if (cFlag)
      result.append(process('c', str));
    if (dFlag)
      result.append(process('d', str));
  }
  return result;
}
...
QString ConsoleCount::process(char flag, QString contents) {
  ...
  switch (flag) {
  case 'a':
    regExp.setPattern("[A-Z][A-Za-z]{4,}");
    message.append("Number of words longer than 4 letters that start with a "
                   "capital letter:");
    break;
  case 'b':
    regExp.setPattern("[A-Za-z]+[-][A-Za-z]+");
    message.append("Number of hyphenated words: ");
    break;
  case 'c':
    regExp.setPattern("^([a-zA-Z])[a-zA-Z]*\\1$");
    message.append("Number of words that start and end on the same letter: ");
    break;
  case 'd':
    regExp.setPattern("^[^aeiouAEIOU].*$");
